...
public interface PostBookmarkRepository extends JpaRepository<PostBookmark, Integer> {
    ...
    /** 사용자별 즐겨찾기한 게시글 조회 */
    List<PostBookmark> findByAccount_AccountId(Integer accountId);
    
    /** 게시글별 즐겨찾기 목록 조회 */
    List<PostBookmark> findByPost_PostId(Integer postId);
    
    /** 특정 사용자의 특정 게시글 즐겨찾기 조회 */
    Optional<PostBookmark> findByPost_PostIdAndAccount_AccountId(Integer postId, Integer accountId);
    ...
    /** 특정 사용자의 즐겨찾기 개수 */
    @Query("SELECT COUNT(pb) FROM PostBookmark pb WHERE pb.account.accountId = :accountId")
